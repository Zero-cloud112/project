...
7. **病歷管理模組**
   - 病歷資料的建立、修改、搜尋、刪除。
   - Dashboard抓取每次判讀後生成的資料夾檔名（原始格式：session_年月日_時分秒）。
   - 醫師可在dashboard上修改檔名為病歷號碼（修改後格式：patient_病歷號碼_年月日）。
   - 支援依時間或關鍵字查詢。
...
sequenceDiagram
    ...
    alt 搜尋病歷
        ...
        臨床研究人員->>系統介面: 選擇session檔案並輸入病歷號碼
        系統介面->>病歷管理模組: 傳送建立/修改病歷請求
        病歷管理模組->>病歷管理模組: 驗證session檔名格式 (session_年月日_時分秒)
        病歷管理模組->>病歷管理模組: 驗證病歷號碼格式
        病歷管理模組->>病歷管理模組: 從session檔名提取日期並生成新檔名 (patient_病歷號碼_年月日)
        病歷管理模組->>檔案系統: 重新命名資料夾 (session_年月日_時分秒 → patient_病歷號碼_年月日)
        檔案系統-->>病歷管理模組: 重新命名完成
        病歷管理模組->>紀錄資料庫: 建立/更新病歷記錄
        紀錄資料庫-->>病歷管理模組: 操作成功
        病歷管理模組-->>系統介面: 回傳病歷資訊
        系統介面-->>臨床研究人員: 顯示病歷建立/修改成功
    else 刪除病歷
        臨床研究人員->>系統介面: 選擇刪除病歷
        系統介面->>臨床研究人員: 確認刪除操作
        臨床研究人員->>系統介面: 確認刪除
...
flowchart TD
    ...
    E -->|建立或修改病歷| P[選擇session檔案]
    P --> Q[輸入病歷號碼]
    Q --> R[驗證session檔名格式<br/>session_年月日_時分秒]
    R --> S{檔名格式正確?}
    S -->|否| T1[顯示檔名格式錯誤]
    T1 --> P
    S -->|是| U[驗證病歷號碼格式]
    ...
    V -->|否| T[顯示病歷號碼格式錯誤]
    T --> Q
    V -->|是| W[從session檔名提取日期]
    W --> X[生成新檔名<br/>patient_病歷號碼_年月日]
    X --> Y[重新命名資料夾<br/>session_年月日_時分秒 → patient_病歷號碼_年月日]
    Y --> Z[建立/更新資料庫記錄]
    Z --> AA{操作成功?}
    AA -->|否| AB[顯示操作失敗]
    AB --> End1
    AA -->|是| AC[顯示病歷建立/修改成功訊息]
    AC --> End2
